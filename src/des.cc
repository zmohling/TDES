...
/* Bit rotation - Key */
int bit_rotation[] = {1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1};
...
void DES::permute(const uint8_t in_size, const uint8_t out_size,
                  const uint8_t *in_block, uint8_t *out_block,
                  const uint8_t *permute_table) {
  int byte, bit;
  for (byte = 0; byte < (out_size / 8); byte++) {
    uint8_t permute_val, t = 0;
    for (bit = 0; bit < 8; bit++) {
      permute_val = *permute_table++;
    }
  }
}
